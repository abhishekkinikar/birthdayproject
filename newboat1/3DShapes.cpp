...
int initialize(VOID)
{
	...
	{
		fprintf(gpFile, " loadGLTexture() for wooden failed");
		uninitialize();
		return -7;
	}

	if (loadGLTexture(&texture_wooden, MAKEINTRESOURCE(IDBITMAP_NEWWOODEN)) == FALSE)
	{
...
VOID display(VOID)
{
	...
	translationMatrix = vmath::translate(0.0f, 0.0f, -6.0f);
	scaleMatrix = vmath::scale(1.5f, 0.1f, 0.5f);
	rotationMatrix_x = rotate(anglecube, 1.0f, 0.0f, 0.0f);
	//rotationMatrix_y = rotate(anglecube, 0.0f, 1.0f, 0.0f);
	//rotationMatrix_z = rotate(anglecube, 0.0f, 0.0f, 1.0f);
	rotationMatrix = rotationMatrix_x * rotationMatrix_y * rotationMatrix_z;
