...
GLfloat angleSaturn = 0.0f, angleAstro = 0.0f, angleMedi = 0.0f, angleComp = 0.0f, angleCfg = 0.0f, speed = 0.2f;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreIntance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	// create window
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("BirthdayDemo"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, HWND_DESKTOP, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
int initialize(void)
{
	...
	if (loadGLTexture(&texture_newwooden, MAKEINTRESOURCE(IDBITMAP_NEWWOODEN)) == FALSE)
	{
		fprintf(gpFile, " loadGLTexture() for newwooden failed");
		uninitialize();
		return -13;
	}
...
void display(void)
{
	...
	if (intro == 1)
	{
		...
		modelMatrix *= vmath::scale(0.8f, 0.1f, 0.2f);
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, texture_newwooden);
		glUniform1i(TSU_boat, 0);
...
void shivling(void)
{
	...
	// draw shivling
	modelMatrix = translate(0.69f + 20.0f, -3.45f + 0.5f, -10.0f - 3.0f);
