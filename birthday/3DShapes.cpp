...
VOID uninitialize(VOID)
{
	...
	if (texture_Birthday)
	{
		glDeleteTextures(1, &texture_Birthday);
		texture_Birthday = 0;
	}
	

	if (VBO_Birthday_position)
	{
		glDeleteBuffers(1, &VBO_Birthday_position);
		VBO_Birthday_position = 0;
	}

	if (VBO_Birthday_texcoord)
	{
		glDeleteBuffers(1, &VBO_Birthday_texcoord);
		VBO_Birthday_texcoord = 0;
	}
	//Deletetion and Uninitialization of VAO
	if (VAO_Birthday)
	{
		glDeleteVertexArrays(1, &VAO_Birthday);
		VAO_Birthday = 0;
	}
	...
	if (SPObj_Birthday)
	{
		...
		glGetProgramiv(SPObj_Birthday, GL_ATTACHED_SHADERS, &numAttachedShaders);
		GLuint* shaderObjects = NULL;
		shaderObjects = (GLuint*)malloc(numAttachedShaders * sizeof(GLuint));
		glGetAttachedShaders(SPObj_Birthday, numAttachedShaders, &numAttachedShaders, shaderObjects);
		for (GLsizei i = 0; i < numAttachedShaders; i++)
		{
			glDetachShader(SPObj_Birthday, shaderObjects[i]);
		...
		shaderObjects = NULL;
		glUseProgram(0);
		glDeleteProgram(SPObj_Birthday);
		SPObj_Birthday = 0;
	}
